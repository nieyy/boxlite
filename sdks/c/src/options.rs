...
fn expand_security_preset(preset: &str, obj: &mut serde_json::Map<String, serde_json::Value>) -> bool {
...
pub unsafe fn options_set_security_json(handle: *mut OptionsHandle, security_json: *const c_char) -> bool {
    unsafe {
        ...
        let mut obj: serde_json::Map<String, serde_json::Value> = match serde_json::from_str(&json_str) {
...
mod security_json_tests {
    ...
    fn maximum_preset_sets_jailer_and_seccomp() {
        ...
        unsafe {
            ...
            assert!(security.seccomp_enabled, "maximum preset must enable seccomp");
            assert!(security.close_fds, "maximum preset must enable close_fds");
            assert!(security.sanitize_env, "maximum preset must enable sanitize_env");
            assert!(security.resource_limits.max_open_files.is_some(), "maximum preset must set max_open_files");
            options_free(handle);
        }
    }
    ...
    fn development_preset_disables_jailer_and_seccomp() {
        unsafe {
            ...
            let security = (*handle).options.advanced.security().clone();
            assert!(!security.jailer_enabled, "development preset must disable jailer");
            assert!(!security.seccomp_enabled, "development preset must disable seccomp");
            assert!(!security.close_fds, "development preset must disable close_fds");
            assert!(!security.sanitize_env, "development preset must disable sanitize_env");
            options_free(handle);
        }
    }
    ...
    fn explicit_field_overrides_preset() {
        ...
        unsafe {
            ...
            let ok = options_set_security_json(handle, json.as_ptr());
            assert!(ok, "preset with explicit override should parse successfully");
            let security = (*handle).options.advanced.security().clone();
            // The explicit jailer_enabled=false takes precedence over maximum's true.
            assert!(!security.jailer_enabled, "explicit jailer_enabled=false must override preset");
            // Other maximum fields are still expanded.
            assert!(security.seccomp_enabled, "seccomp_enabled comes from maximum preset");
            options_free(handle);
        }
    }
    ...
    fn concrete_json_without_preset_still_works() {
        unsafe {
            ...
            let ok = options_set_security_json(handle, json.as_ptr());
            assert!(ok, "concrete security JSON without preset should parse successfully");
            let security = (*handle).options.advanced.security().clone();
    ...
    fn returns_false_for_camelcase_unknown_field() {
        ...
        unsafe {
            ...
            let ok = options_set_security_json(handle, json.as_ptr());
            assert!(!ok, "camelCase key seccompEnabled is unknown — must return false, not silently ignore it");
            options_free(handle);
        }
    }
    ...
    fn returns_false_for_unknown_preset() {
        ...
        unsafe {
            ...
            let ok = options_set_security_json(handle, json.as_ptr());
            assert!(!ok, "unknown preset 'maximun' must return false, not silently apply standard");
            options_free(handle);
        }
    }
}
