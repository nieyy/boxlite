...
// Linux signal number. The box always runs a Linux guest, so these must be
...
// default. user is the OS user inside the guest (e.g., "root", "boxlite",
// "1000:1000"); empty inherits the container image default. Service.sshUserOrDefault()
// is the canonical source for the user argument: it defaults to "root" (the
// typical user for standard container images) when sshUser is not configured.
// Tests inject a stub via Service.startExec.
type startExecFn func(ctx context.Context, boxId, cmd string, args []string, stdout, stderr io.Writer, tty bool, env map[string]string, user string) (sshExecution, error)
...
type Service struct {
	...
	// Empty means "root" (the typical user for standard container images).
	// Set explicitly to restrict to a non-root unix user.
...
// sshUserOrDefault returns sshUser if set, otherwise "root".
// Root is the correct default for standard container images (ubuntu, python:slim,
// alpine, etc.) where root is the typical container user. Callers that need a
// restricted unix_user must set sshUser explicitly in the request.
...
func NewService(logger *slog.Logger, boxlite *blclient.Client) *Service {
	...
	service.startExec = func(ctx context.Context, boxId, cmd string, args []string, stdout, stderr io.Writer, tty bool, env map[string]string, user string) (sshExecution, error) {
		drained := make(chan struct{})
		onExit := func(_ int) { close(drained) }
		exec, err := boxlite.StartExecution(ctx, boxId, cmd, args, stdout, stderr, tty, env, user, onExit)
		if err != nil {
			return nil, err
		}
...
func (s *Service) handleConnection(conn net.Conn, serverConfig *ssh.ServerConfig) {
	...
	for newChannel := range chans {
		go s.handleChannel(newChannel, boxId)
	}
}
...
func (s *Service) handleChannel(newChannel ssh.NewChannel, boxId string) {
	...
	// proxy path to <boxId>:22220 that is not available via the BoxLite
	...
	if err != nil {
		s.log.Warn("Could not accept client channel", "boxID", boxId, "error", err)
		return
	}
	defer ch.Close()
	...
	for req := range reqs {
		...
		case "pty-req":
			...
			if err := ssh.Unmarshal(req.Payload, &ptyPayload); err != nil {
				s.log.Debug("Failed to parse pty-req payload", "boxID", boxId, "error", err)
			} else {
		...
		case "exec":
			...
			if err := ssh.Unmarshal(req.Payload, &msg); err != nil {
				s.log.Warn("Failed to parse exec payload", "boxID", boxId, "error", err)
				if req.WantReply {
			...
			if !tty {
				s.log.Debug("Rejecting non-PTY exec (exec pipeline is text-only)", "boxID", boxId, "cmd", msg.Command)
			...
			if err := s.runExec(ctx, cancel, ch, reqs, boxId, "/bin/sh", []string{"-c", msg.Command}, tty, req, initialRows, initialCols, termEnv, s.sshUserOrDefault()); err != nil {
				s.log.Warn("Exec failed to start", "boxID", boxId, "error", err)
			}
			return
		...
		case "shell":
			...
			if !tty {
				s.log.Debug("Rejecting non-PTY shell (exec pipeline is text-only)", "boxID", boxId)
				_, _ = fmt.Fprintf(ch.Stderr(),
			...
			if err := s.runExec(ctx, cancel, ch, reqs, boxId, "/bin/sh", nil, tty, req, initialRows, initialCols, termEnv, s.sshUserOrDefault()); err != nil {
				s.log.Warn("Shell failed to start", "boxID", boxId, "error", err)
			}
			return
		...
		case "subsystem":
			...
			if err := ssh.Unmarshal(req.Payload, &msg); err != nil {
				s.log.Warn("Failed to parse subsystem payload", "boxID", boxId, "error", err)
				if req.WantReply {
					_ = req.Reply(false, nil)
				}
				return
			...
			// corrupted data.
			s.log.Debug("Subsystem not supported over exec bridge (binary stream limitation)", "name", msg.Name, "boxID", boxId)
			if req.WantReply {
				_ = req.Reply(false, nil)
			}
			return
		...
		default:
			s.log.Debug("Unhandled pre-exec request", "type", req.Type, "boxID", boxId)
			if req.WantReply {
				if err := req.Reply(false, nil); err != nil {
...
// runExec starts a command in the box via boxlite exec and bridges it to the SSH channel.
...
func (s *Service) runExec(
	...
	s.log.Info("Starting exec in box", "boxID", boxId, "cmd", cmd, "tty", tty)
	...
	if n := s.inFlightStartups.Add(1); n > s.maxInFlightStartupsOrDefault() {
		...
		s.log.Warn("Rejecting SSH exec: too many in-flight startups", "boxID", boxId, "inFlight", n-1, "max", s.maxInFlightStartupsOrDefault())
	...
	go func() {
		...
		exec, err := s.startExec(ctx, boxId, cmd, args, ch, ch.Stderr(), tty, env, user)
	...
	case <-startCtx.Done():
		...
		s.log.Warn("Failed to start execution in box", "boxID", boxId, "error", err)
		if triggerReq.WantReply {
			_ = triggerReq.Reply(false, nil)
		}
		return err
	case r := <-resultCh:
		if r.err != nil {
			s.log.Warn("Failed to start execution in box", "boxID", boxId, "error", r.err)
			if triggerReq.WantReply {
				_ = triggerReq.Reply(false, nil)
			}
			return r.err
	...
	if tty && initialRows > 0 && initialCols > 0 {
		...
		if err := execution.ResizeTTY(resizeCtx, initialRows, initialCols); err != nil {
			s.log.Debug("Failed to apply initial PTY dimensions", "boxID", boxId, "error", err)
		}
		resizeCancel()
	}
		...
		for {
			...
			if n > 0 {
				if _, writeErr := execStdin.Write(buf[:n]); writeErr != nil {
					...
					s.log.Debug("Execution stdin write error", "boxID", boxId, "error", writeErr)
					break
				}
			}
			if readErr != nil {
				...
				case "window-change":
					...
					if len(req.Payload) >= 8 {
						...
						if rows > 0 && cols > 0 {
							...
							resizeCancel()
							if err != nil {
								s.log.Debug("Failed to resize TTY", "boxID", boxId, "error", err)
							}
						}
					}
				case "signal":
					...
					if err := ssh.Unmarshal(req.Payload, &msg); err != nil {
						s.log.Debug("Failed to parse signal payload", "boxID", boxId, "error", err)
						break
					}
					sig := signalFromName(msg.Signal)
					if sig == 0 {
						s.log.Debug("Ignoring unknown SSH signal", "signal", msg.Signal, "boxID", boxId)
						break
					}
					killCtx, killCancel := context.WithTimeout(context.Background(), 5*time.Second)
					if err := execution.Signal(killCtx, sig); err != nil {
						s.log.Debug("Failed to forward signal", "signal", msg.Signal, "boxID", boxId, "error", err)
					}
					killCancel()
				default:
					s.log.Debug("Ignoring in-session request", "type", req.Type, "boxID", boxId)
				}
				if req.WantReply {
					if err := req.Reply(false, nil); err != nil {
	...
	if waitErr != nil {
		s.log.Debug("Execution wait ended", "boxID", boxId, "error", waitErr)
	}
	...
	if ctx.Err() != nil {
		...
		if err := execution.Kill(killCtx); err != nil {
			s.log.Debug("Failed to kill guest process after disconnect", "boxID", boxId, "error", err)
		}
		killCancel()
	}

	s.log.Info("Exec completed", "boxID", boxId, "exitCode", exitCode)
	...
	if waitErr == nil && ctx.Err() == nil {
		...
		case <-time.After(drainTimeout):
			...
			s.log.Warn("Drain timeout waiting for stdout pump; closing channel and killing guest process",
				"boxID", boxId, "timeout", drainTimeout)
			_ = ch.Close() // unblock any stuck ch.Write in the SDK drain goroutine
			killCtx, killCancel := context.WithTimeout(context.Background(), 10*time.Second)
			if err := execution.Kill(killCtx); err != nil {
				s.log.Debug("Kill after drain timeout failed", "boxID", boxId, "error", err)
			}
			killCancel()
	...
	if waitErr == nil {
		...
		if _, err := ch.SendRequest("exit-status", false, payload); err != nil {
			s.log.Debug("Failed to send exit-status", "boxID", boxId, "error", err)
		}
	}
