...
// TestSSHUserDefaultsToBoxlite is the key invariant guard for the SSH user
// policy: when Service.sshUser is empty, exec and shell requests must be
// launched with user="boxlite", not user="" (which inherits the image default,
// typically root for python:slim, alpine, and other standard images).
//
...
func TestSSHUserDefaultsToBoxlite(t *testing.T) {
	t.Parallel()
	...
	// inherits the container image default, which is root for many standard images.
	// The policy default must always be "boxlite".
	if capturedUser == "" {
		t.Error("startExec was called with user=\"\" (empty): " +
			"empty user inherits the container image default, which is root for python:slim, alpine, " +
			"and other standard images; the SSH gateway must always pass an explicit unprivileged user " +
			"(default: \"boxlite\") to prevent unintentional root access")
	}
	if capturedUser != "boxlite" {
		t.Errorf("startExec was called with user=%q; want \"boxlite\" (the SSH gateway default)", capturedUser)
	}
}
