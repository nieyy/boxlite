...
func (f *fakeSSHBox) AdminSockPath() string { return "" }

var _ sshCapable = (*fakeSSHBox)(nil)

// gvproxyAdminSocket is a test-local helper that reconstructs the socket path
// from homeDir + boxID. It mirrors the path layout that boxlite uses, and is
// kept here (rather than in production code) so tests that set up fake
// gvproxy servers can construct the expected socket path without depending on
// a live boxlite handle.
func gvproxyAdminSocket(homeDir, boxID string) string {
	return filepath.Join(homeDir, "boxes", boxID, "sockets", "gvproxy-ctl.sock")
}
...
func TestEnableRollbackUnexposeFailed(t *testing.T) {
	...
	// exists at boxes/<boxId>/sockets/gvproxy-ctl.sock; we lock the parent.
	boxDir := filepath.Join(homeDir, "boxes", boxId)
