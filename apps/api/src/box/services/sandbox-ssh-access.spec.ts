...
/** Build a minimal Box stub with only the fields BoxService inspects. */
function makeBox(id: string): Box {
  const s = new Box('us', id)
  s.id = id
  s.organizationId = 'org-1'
  s.name = id
  s.region = 'us'
  s.state = BoxState.STARTED
  s.desiredState = BoxDesiredState.STARTED
...
/**
 * Build a BoxService instance with the minimum mocks needed to exercise
...
function buildService(overrides?: { redisLockProvider?: Partial<RedisLockProvider> }) {
  ...
  // box id directly so both the id-lookup and name-lookup return the stub.
  const boxRepo = {
    findOne: jest.fn().mockImplementation(({ where }) => {
      // findOneByIdOrName calls with { id: boxIdOrName, ... } first,
      // then { name: boxIdOrName, ... } on fallback.
      const id = where?.id ?? where?.name
      if (id) {
        return Promise.resolve(makeBox(id))
      }
  ...
  const warmPoolService = { fetchWarmPoolBox: jest.fn().mockResolvedValue(null) }
  ...
  const boxActivityService = { updateLastActivityAt: jest.fn() }
  ...
  const service = new BoxService(
    ...
    boxLookupCacheInvalidationService as any,
...
describe('BoxService SSH access lock contract', () => {
  const BOX_ID = 'box-uuid-a'
  const EXPECTED_LOCK_KEY = `box:${BOX_ID}:ssh-access`

  describe('createSshAccess', () => {
    it('acquires the per-box SSH lock with the expected key (owned)', async () => {
      const { service, redisLockProvider } = buildService()

      await service.createSshAccess(BOX_ID, 60, 'org-1')

      expect(redisLockProvider.waitForLockOwned).toHaveBeenCalledWith(EXPECTED_LOCK_KEY, expect.any(Number))
    })

    it('releases the per-box SSH lock via compare-and-delete after creating access', async () => {
      const { service, redisLockProvider } = buildService()
    ...
    it('releases the lock via compare-and-delete even when sshAccessRepository.save throws', async () => {
      ...
      await expect(svcWithFailingSave.createSshAccess(BOX_ID, 60, 'org-1')).rejects.toThrow('DB write failed')
    ...
    it('aborts without saving a DB token when lock expired during runner call (fencing check)', async () => {
      ...
      await expect(service.createSshAccess(BOX_ID, 60, 'org-1')).rejects.toThrow(
    ...
    it('does NOT call disableSSHAccess when isLockOwned returns false (lost-lock cleanup must not clobber a concurrent create)', async () => {
      ...
      //
      // To exercise this path we need a real runnerId on the box and a
      ...
      // Build a service with a box that has a runnerId so the runner path runs.
      const boxWithRunner = makeBox(BOX_ID)
      boxWithRunner.runnerId = 'runner-1'

      const boxRepo = {
        findOne: jest.fn().mockResolvedValue(boxWithRunner),
        find: jest.fn().mockResolvedValue([]),
        findAndCount: jest.fn().mockResolvedValue([[], 0]),
      ...
      const boxLookupCacheInvalidationService = { invalidateOrgId: jest.fn(), invalidate: jest.fn() }
      const boxActivityService = { updateLastActivityAt: jest.fn() }
      ...
      await expect(service.createSshAccess(BOX_ID, 60, 'org-1', [], 'boxlite')).rejects.toThrow(
        'SSH access revoked during creation',
      )
    ...
    it('does NOT call disableSSHAccess on save failure when prior active tokens exist with same unix_user (Finding 2, Round 49)', async () => {
      ...
      //
      // Scenario: box already has an active SSH token (rotation, same user).
      ...
      const sshAccessRepository = {
        ...
        findOne: jest.fn().mockResolvedValue(null),
        find: jest.fn().mockResolvedValue([{ id: 'prior-token-uuid', unixUser: 'boxlite', boxId: BOX_ID }]),
      }
      const ownedToken = new LockCode('test-lock-token')
      const redisLockProvider = {
        waitForLock: jest.fn().mockResolvedValue(undefined),
      ...
      await expect(service.createSshAccess(BOX_ID, 60, 'org-1', [], 'boxlite')).rejects.toThrow('DB write failed')
    ...
    it('DOES call disableSSHAccess on save failure when prior active tokens exist but unix_user changed (Finding [high], Round 51)', async () => {
      ...
      //
      // Scenario: box has an active token for unix_user='alice'. A new
      ...
      const sshAccessRepository = {
        ...
        count: jest.fn().mockResolvedValue(1),
        findOne: jest.fn().mockResolvedValue(null),
        find: jest.fn().mockResolvedValue([{ id: 'prior-token-uuid', unixUser: 'alice', boxId: BOX_ID }]),
      }
      const ownedToken = new LockCode('test-lock-token')
      const redisLockProvider = {
        waitForLock: jest.fn().mockResolvedValue(undefined),
      ...
      await expect(service.createSshAccess(BOX_ID, 60, 'org-1', [], 'bob')).rejects.toThrow('DB write failed')
    ...
    it('does NOT call disableSSHAccess when enableSSHAccess fails and prior real-SSH tokens exist (Finding [high], Round 10)', async () => {
      ...
      //
      // Scenario: box has an existing real-SSH token (unixUser='boxlite').
      ...
      const sshAccessRepository = {
        ...
        count: jest.fn().mockResolvedValue(1),
        findOne: jest.fn().mockResolvedValue(null),
        find: jest.fn().mockResolvedValue([{ id: 'prior-real-ssh-uuid', unixUser: 'boxlite', boxId: BOX_ID }]),
      }
      const ownedToken = new LockCode('test-lock-token')
      const redisLockProvider = {
        waitForLock: jest.fn().mockResolvedValue(undefined),
      ...
      await expect(service.createSshAccess(BOX_ID, 60, 'org-1', [], 'boxlite')).rejects.toThrow('Runner unreachable')
    ...
    it('calls disableSSHAccess when delete-old-tokens fails after unix_user change (Finding 1, Round 52)', async () => {
      ...
      // Request with unix_user='bob' — different from prior token's 'alice'.
      await expect(service.createSshAccess(BOX_ID, 60, 'org-1', [], 'bob')).rejects.toThrow('DB delete failed')
    ...
    it('DOES call disableSSHAccess on save failure when newest prior token matches requested user but older tokens belong to a different user (Finding 2, Round 53)', async () => {
      ...
      const sshAccessRepository = {
        ...
        findOne: jest.fn().mockResolvedValue(null),
        find: jest.fn().mockResolvedValue([
          { id: 'alice-token-uuid', unixUser: 'alice', boxId: BOX_ID },
          { id: 'bob-token-uuid', unixUser: 'bob', boxId: BOX_ID },
        ]),
      }
      const ownedToken = new LockCode('test-lock-token')
      const redisLockProvider = {
    ...
    it('does NOT call disableSSHAccess when delete-old-tokens fails with same unix_user (Finding 1, Round 52)', async () => {
      ...
      await expect(service.createSshAccess(BOX_ID, 60, 'org-1', [], 'boxlite')).rejects.toThrow('DB delete failed')
    ...
    it('accepts real-SSH requests (unix_user != null) for v2 runners and calls enableSSHAccess (Round 56 updated Round 66, revised Round 5)', async () => {
      ...
      // BadRequestError for v2+unixUser was removed from box.service.ts.
      //
      ...
      const boxWithV2Runner = makeBox(BOX_ID)
      boxWithV2Runner.runnerId = 'runner-v2'

      const boxRepo = {
        findOne: jest.fn().mockResolvedValue(boxWithV2Runner),
        find: jest.fn().mockResolvedValue([]),
        findAndCount: jest.fn().mockResolvedValue([[], 0]),
        insert: jest.fn().mockImplementation((s: any) => Promise.resolve(s)),
        update: jest.fn().mockImplementation((_: any, { entity }: any) => Promise.resolve(entity)),
      ...
      await service.createSshAccess(BOX_ID, 60, 'org-1', [], 'alice')
      expect(enableSSHAccess).toHaveBeenCalledTimes(1)
      expect(sshAccessRepository.save).toHaveBeenCalledTimes(1)
      ...
      await service.createSshAccess(BOX_ID, 60, 'org-1', [], null)
    ...
    it('rejects real-SSH requests (unix_user != null) for runner-less boxes with BadRequestError (Finding 2, Round 66)', async () => {
      // Reproducer for Finding 2, Round 66:
      // A box with no runnerId and unix_user='boxlite' must fail with
      // BadRequestError — there is no runner to configure SSH on.
      // unixUser=null (exec-bridge) must still succeed regardless of runner.
      const { service, sshAccessRepository } = buildService()

      // box from buildService() has no runnerId (findOne returns null for runnerService).
      await expect(service.createSshAccess(BOX_ID, 60, 'org-1', [], 'boxlite')).rejects.toThrow(
    ...
    it('rejects real-SSH requests with distinct error when runnerId is set but runner record is not found (Finding 4, Round 67)', async () => {
      ...
      // box.runnerId is set (non-null) but runnerService.findOne returns null
      // (orphaned runner reference — runner was deleted after box was created).
      // The !runner guard must produce "runner not found" NOT "v2 runners not supported".
      const boxWithOrphanedRunner = makeBox(BOX_ID)
      boxWithOrphanedRunner.runnerId = 'deleted-runner-id'
      ...
      // orphaned runner: runnerId set on box but findOne returns null
      const runnerService = {
        findOne: jest.fn().mockResolvedValue(null),
        findOneOrFail: jest.fn().mockResolvedValue(null),
      ...
      try {
        await service.createSshAccess(BOX_ID, 60, 'org-1', [], 'boxlite')
      } catch (e: any) {
        caughtErr = e
      }
    ...
    it('skips delete-old-tokens and returns the new token when lock is lost between save and delete (Finding 2, Round 57)', async () => {
      ...
      const result = await service.createSshAccess(BOX_ID, 60, 'org-1')
    ...
    it('propagates a clear error when the runner returns 503 (SSH gateway not configured, Round 7)', async () => {
      ...
      // error (AxiosError). The box service propagated that as an opaque 500.
      //
      ...
      // box.service.ts forwards that Error to the caller unchanged, so the
      ...
      const boxWithRunner = makeBox(BOX_ID)
      boxWithRunner.runnerId = 'runner-v2'
      ...
      await expect(service.createSshAccess(BOX_ID, 60, 'org-1', [], 'alice')).rejects.toThrow(
    ...
    it('calls disableSSHAccess when rotating from real-SSH token to legacy (null) token (Finding [high], Round 68)', async () => {
      ...
      //
      // Scenario: box has an active real-SSH token (unixUser='boxlite').
      ...
      // deleting the old real-SSH tokens (under the same per-box lock).
      const disableSSHAccess = jest.fn().mockResolvedValue(undefined)
      const enableSSHAccess = jest.fn().mockResolvedValue(undefined)
      const runnerAdapter = { enableSSHAccess, disableSSHAccess }
  ...
  describe('revokeSshAccess', () => {
    it('acquires the per-box SSH lock with the expected key (owned)', async () => {
      ...
      await service.revokeSshAccess(BOX_ID, undefined, 'org-1')

      expect(redisLockProvider.waitForLockOwned).toHaveBeenCalledWith(EXPECTED_LOCK_KEY, expect.any(Number))
    })

    it('releases the per-box SSH lock via compare-and-delete after revoking access', async () => {
      const { service, redisLockProvider } = buildService()
  ...
  describe('lock key invariants', () => {
    it('createSshAccess and revokeSshAccess use the SAME lock key for the same box', async () => {
      ...
      await createSvc.createSshAccess(BOX_ID, 60, 'org-1')
      await revokeSvc.revokeSshAccess(BOX_ID, undefined, 'org-1')
    ...
    it('different boxes use different lock keys', async () => {
      const keysA: string[] = []
      const keysB: string[] = []
      ...
      await svcA.createSshAccess('box-a', 60, 'org-1')
      await svcB.createSshAccess('box-b', 60, 'org-1')

      expect(keysA[0]).not.toBe(keysB[0])
      expect(keysA[0]).toContain('box-a')
      expect(keysB[0]).toContain('box-b')
    })
    ...
    it('lock key does not collide with the state-change lock key', async () => {
      ...
      const stateChangeKey = `box:${BOX_ID}:state-change`
      expect(keysUsed).not.toContain(stateChangeKey)
    })

    it('lock key matches expected pattern box:<id>:ssh-access', async () => {
      const keysUsed: string[] = []
      const token = new LockCode('t')
      ...
      expect(keysUsed[0]).toBe(`box:${BOX_ID}:ssh-access`)
    })
  })
})

// ---------------------------------------------------------------------------
// Round 41, Finding 1: validateSshAccess must hold the per-box SSH lock
 ...
 */
function makeExpiredSshAccess(boxId: string, withRunner = false, nullBox = false): Record<string, unknown> {
  return {
    id: 'ssh-access-uuid',
    token: 'expired-token',
    boxId,
    ...
    box: nullBox
      ? null
      : withRunner
        ? { id: boxId, runnerId: 'runner-1', region: 'us' }
        : { id: boxId, runnerId: null, region: 'us' },
  }
}
...
function buildValidateService(opts: {
  ...
  const { activeCount = 0, withRunner = false, nullBox = false } = opts
  const boxId = VALIDATE_BOX_ID

  const expiredAccess = makeExpiredSshAccess(boxId, withRunner, nullBox)
  ...
  const boxRepo = {
    findOne: jest.fn().mockResolvedValue(makeBox(boxId)),
    find: jest.fn().mockResolvedValue([]),
    findAndCount: jest.fn().mockResolvedValue([[], 0]),
    insert: jest.fn().mockImplementation((s) => Promise.resolve(s)),
    update: jest.fn().mockImplementation((_, { entity }) => Promise.resolve(entity)),
...
const VALIDATE_BOX_ID = 'box-validate-uuid'
const EXPECTED_SSH_LOCK_KEY = `box:${VALIDATE_BOX_ID}:ssh-access`

describe('BoxService validateSshAccess lock contract (Round 41, Finding 1)', () => {
  it('acquires the per-box SSH lock before calling disableSSHAccess on expired token with 0 active tokens', async () => {
    ...
    // with the correct per-box SSH lock key.
  ...
  it('releases the per-box SSH lock via compare-and-delete after disable', async () => {
  ...
  it('calls disableSSHAccess and returns invalid when no active tokens remain (lock held)', async () => {
    ...
    expect(disableSSHAccess).toHaveBeenCalledTimes(1)
    expect(result).toEqual({ valid: false, boxId: '', unixUser: null })
  })

  it('skips lock and disableSSHAccess when box has no runner, but still deletes the expired row (Finding 1, Round 69)', async () => {
    ...
    // (exec-bridge tokens for runner-less boxes would otherwise accumulate forever).
  ...
  it('deletes expired row when box relation is null (concurrently deleted), without lock (Finding 2, Round 70)', async () => {
    // Reproducer: sshAccess.box is null when the box was deleted concurrently.
    // Before fix: the else-if (sshAccess.box) branch was skipped entirely (null
    // is falsy), so the orphaned SSH-access row was never deleted — unbounded DB growth.
    // After fix: the else branch deletes the row even when box is null.
    const { service, redisLockProvider, disableSSHAccess, sshAccessRepository } = buildValidateService({
      nullBox: true,
    })
    ...
    expect(redisLockProvider.waitForLockOwned).not.toHaveBeenCalled()
    expect(disableSSHAccess).not.toHaveBeenCalled()
    // KEY ASSERTION: the expired row must be deleted even when box is null.
    expect(sshAccessRepository.delete).toHaveBeenCalledWith({ id: 'ssh-access-uuid' })
  })

  it('returns invalid for a valid (non-expired) token when box relation is null (concurrently deleted) (Finding 1, Round 71)', async () => {
    // Reproducer: sshAccess.box is null and the token has NOT yet expired.
    // Before fix: the non-expired fast path returned `sshAccess.box.id` without
    // a null check — a TypeError crash propagated to the gateway, which treated it
    // as a failed validation (non-200 response) and closed the connection. Any box
    // being deleted while a valid token existed would trigger this crash.
    // After fix: the non-expired path guards `if (!sshAccess.box)` and returns
    // { valid: false, boxId: '', unixUser: null } cleanly.
    const boxId = VALIDATE_BOX_ID
    const validAccessNullBox = {
      id: 'ssh-access-uuid',
      token: 'valid-token',
      boxId,
      unixUser: null,
      expiresAt: new Date(Date.now() + 1000 * 60 * 60), // 1 hour from now
      box: null, // box concurrently deleted — TypeORM resolves FK to null
    }

    const sshAccessRepository = {
      findOne: jest.fn().mockResolvedValue(validAccessNullBox),
      count: jest.fn().mockResolvedValue(0),
      delete: jest.fn().mockResolvedValue({ affected: 0 }),
      save: jest.fn(),
    }
    ...
    // Override findOne so it returns the null-box non-expired access.
  ...
  it('deletes the expired token row so a second call returns invalid without acquiring lock or calling disableSSHAccess', async () => {
    ...
    const boxId = VALIDATE_BOX_ID
    const expiredAccess = makeExpiredSshAccess(boxId, true)
    ...
    expect(result1).toEqual({ valid: false, boxId: '', unixUser: null })
    ...
    expect(result2).toEqual({ valid: false, boxId: '', unixUser: null })
  ...
  it('returns the token unix_user in the response so the gateway can detect rotation mismatches (Finding 1, Round 53)', async () => {
    ...
    const validAccess = {
      ...
      unixUser: 'alice',
      expiresAt: new Date(Date.now() + 1000 * 60 * 60), // 1 hour from now
      box: { id: boxId, runnerId: 'runner-1', region: 'us' },
    }
...
describe('BoxService validateSshAccess null unixUser propagation (Round 57, Finding 1)', () => {
  it('returns unixUser=null for a valid token with null unixUser so HasUnixUser() is false in the Go client', async () => {
    ...
    const validAccessNullUser = {
      ...
      box: { id: boxId, runnerId: 'runner-v2', region: 'us' },
    }
...
describe('revokeSshAccess runner-disable fencing (Round 58, Finding 1)', () => {
  const REVOKE_BOX_ID = 'box-revoke-r58'
  const REVOKE_LOCK_KEY = `box:${REVOKE_BOX_ID}:ssh-access`
  ...
  function buildRevokeService(opts: {
    ...
    tokensToRevoke?: Array<{ id: string; unixUser: string | null; boxId: string }>
    /** Whether runnerService.findOne resolves the runner record. Defaults to true;
     *  set to false to simulate a missing/deregistered runner. */
    runnerFound?: boolean
  }) {
    ...
    const boxWithRunner = makeBox(REVOKE_BOX_ID)
    boxWithRunner.runnerId = 'runner-r58'
    ...
    const runnerService = {
      findOne: jest.fn().mockResolvedValue(opts.runnerFound === false ? null : { id: 'runner-r58', apiVersion: '1' }),
      findOneOrFail: jest.fn().mockResolvedValue(null),
      getRandomAvailableRunner: jest.fn().mockRejectedValue(new Error('No available runners')),
    }
    const runnerAdapterFactory = { create: jest.fn().mockResolvedValue(runnerAdapter) }
  ...
  it('skips disableSSHAccess when lock expires before runner call (lock-lost fencing)', async () => {
    ...
    await service.revokeSshAccess(REVOKE_BOX_ID, undefined, 'org-1')
  ...
  it('calls disableSSHAccess when lock is still owned before runner call (normal revoke)', async () => {
    ...
    const { service } = buildRevokeService({
      ...
      disableSSHAccess,
      tokensToRevoke: [{ id: 'token-id', unixUser: 'boxlite', boxId: REVOKE_BOX_ID }],
    })
  ...
  it('skips disableSSHAccess when another valid real-SSH token remains after per-token delete (Round 63, Finding 2)', async () => {
    ...
    const { service } = buildRevokeService({
      ...
      tokensToRevoke: [{ id: 'token-a-id', unixUser: 'boxlite', boxId: REVOKE_BOX_ID }],
    })

    await service.revokeSshAccess(REVOKE_BOX_ID, 'token-a', 'org-1')
  ...
  it('calls disableSSHAccess when per-token delete removes the last token (Round 63, Finding 2)', async () => {
    ...
    const { service } = buildRevokeService({
      ...
      tokensToRevoke: [{ id: 'last-token-id', unixUser: 'boxlite', boxId: REVOKE_BOX_ID }],
    })

    await service.revokeSshAccess(REVOKE_BOX_ID, 'last-token', 'org-1')
  ...
  it('should disable runner SSH when last real-SSH token is revoked even if legacy token remains', async () => {
    ...
    //
    // Scenario: box has one real-SSH token (unixUser='boxlite') and one legacy
    ...
    //   3. OLD BUG: count({ boxId }) returns 1 (counts the legacy row)
    ...
    const { service } = buildRevokeService({
      ...
      tokensToRevoke: [{ id: 'real-token-id', unixUser: 'boxlite', boxId: REVOKE_BOX_ID }],
    })

    await service.revokeSshAccess(REVOKE_BOX_ID, 'real-ssh-token', 'org-1')
  ...
  it('should call disableSSHAccess when last legacy token is revoked and no real-SSH row remains (Round 6, Finding [high])', async () => {
    ...
    const { service } = buildRevokeService({
      ...
      tokensToRevoke: [{ id: 'legacy-token-id', unixUser: null, boxId: REVOKE_BOX_ID }],
    })

    await service.revokeSshAccess(REVOKE_BOX_ID, 'legacy-token', 'org-1')
  ...
  it('explicit revoke should fail and preserve DB tokens when runner disableSSHAccess fails (Round 12, Finding [high])', async () => {
    ...
    await expect(service.revokeSshAccess(REVOKE_BOX_ID, undefined, 'org-1')).rejects.toThrow(disableError)
  ...
  it('fails closed and preserves DB tokens when the runner record is not found during a real-SSH revoke', async () => {
    // Reproducer: revokedIsRealSsh=true but runnerService.findOne(box.runnerId)
    // resolves null (e.g. the runner was deregistered/deleted). The old code only
    // guarded `if (runner) { ... }` around the disableSSHAccess call — when runner
    // is falsy, that whole block is skipped silently and execution falls through
    // to the DB delete below, regardless of revokedIsRealSsh. Result: DB says
    // revoked, but the runner-side sshd/port-forward (if the runner comes back)
    // is never told to disable — a false revocation.
    //
    // Fix: when revokedIsRealSsh is true and the runner can't be found, throw
    // instead of silently falling through to the delete.
    const disableSSHAccess = jest.fn().mockResolvedValue(undefined)
    const { service, sshAccessRepository } = buildRevokeService({
      isLockOwned: () => Promise.resolve(true),
      disableSSHAccess,
    ...
    // KEY ASSERTION 1: revokeSshAccess must throw rather than silently succeed.
    await expect(service.revokeSshAccess(REVOKE_BOX_ID, undefined, 'org-1')).rejects.toThrow()

    // KEY ASSERTION 2: DB tokens must NOT have been deleted.
    expect(sshAccessRepository.delete).not.toHaveBeenCalled()

    // disableSSHAccess itself was never reachable (no adapter could be built
    // without a runner record) — it must not have been called either.
    expect(disableSSHAccess).not.toHaveBeenCalled()
  })
  ...
  it('legacy token revocation succeeds even when runner disableSSHAccess fails (Round 13, Finding [high])', async () => {
    ...
    const { service, sshAccessRepository } = buildRevokeService({
      ...
      realSshRemainingCount: 0,
      // The only token being revoked is a legacy exec-bridge token (unixUser=null).
      tokensToRevoke: [{ id: 'legacy-only-id', unixUser: null, boxId: REVOKE_BOX_ID }],
    })

    // KEY ASSERTION 1: revokeSshAccess must NOT throw — legacy token revocation is
    // independent of runner availability.
    await expect(service.revokeSshAccess(REVOKE_BOX_ID, 'legacy-token', 'org-1')).resolves.toBeDefined()
...
describe('BoxService validateSshAccess expiry-cleanup ordering (Round 61, Finding 2)', () => {
...
describe('createSshAccess controller unixUser passthrough (Round 61, Finding 3)', () => {
  it('passes unix_user from request body to boxService.createSshAccess', async () => {
...
describe('validateSshAccess controller unixUser wire propagation (Round 62, Finding 1)', () => {
  it('includes unixUser in the HTTP response for a real-SSH token', () => {
    ...
    // result.boxId) — no third argument. fromValidationResult sets
    ...
    //
    // Fix: fromValidationResult(result.valid, result.boxId, result.unixUser).
    //
    ...
    const withUser = SshAccessValidationDto.fromValidationResult(true, 'box-1', 'alice')
  ...
  it('includes unixUser=null in the HTTP response for a legacy exec-bridge token', () => {
    ...
    const withNull = SshAccessValidationDto.fromValidationResult(true, 'box-2', null)
...
describe('revokeSshAccess best-effort disable on runner 503 (Round 9, Finding 1)', () => {
  const BOX_9 = 'box-r9-f1'
  const LOCK_KEY_9 = `box:${BOX_9}:ssh-access`
  ...
  it('propagates runner 503 to the caller during revoke and preserves DB tokens (Round 12 updated)', async () => {
    ...
    const disableSSHAccess = jest.fn().mockRejectedValue(
      new Error('disableSSHAccess failed for box box-r9-f1 on runner runner-r9: HTTP 503'),
    )
    const enableSSHAccess = jest.fn().mockResolvedValue(undefined)
    const runnerAdapter = { enableSSHAccess, disableSSHAccess }

    const boxWithRunner = makeBox(BOX_9)
    boxWithRunner.runnerId = 'runner-r9'
    ...
    const sshAccessRepository = {
      ...
      findOne: jest.fn().mockResolvedValue(null),
      find: jest.fn().mockResolvedValue([{ id: 'token-r9', unixUser: 'boxlite', boxId: BOX_9 }]),
    }
    ...
    await expect(service.revokeSshAccess(BOX_9, undefined, 'org-1')).rejects.toThrow('HTTP 503')
