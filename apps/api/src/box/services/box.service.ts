...
export class BoxService {
  ...
  private sshLockKey(boxId: string): string {
    return `box:${boxId}:ssh-access`
  }
  ...
  async createSshAccess(
    ...
    const box = await this.findOneByIdOrName(boxIdOrName, organizationId)
    const lockKey = this.sshLockKey(box.id)

    const lockToken = await this.redisLockProvider.waitForLockOwned(lockKey, 90)
    try {
      const priorTokens = await this.sshAccessRepository.find({ where: { boxId: box.id } })
      ...
      if (unixUser !== null) {
        ...
        if (!box.runnerId) {
          throw new BadRequestError('Real-SSH access (unix_user) requires a runner; box has none attached')
        }
        const runner = await this.runnerService.findOne(box.runnerId)
        if (!runner) {
        ...
        try {
          await adapter.enableSSHAccess(box.id, unixUser)
          ...
          if (!hadPriorRealSSHTokens) {
            try {
              await adapter.disableSSHAccess(box.id)
            } catch (disableErr) {
      ...
      } else if (hadPriorRealSSHTokens && box.runnerId) {
      ...
      try {
        ...
        if (runnerSSHEnabled && runnerAdapterForRollback && (!hadPriorActiveSshAccess || unixUserChanged)) {
          try {
            await runnerAdapterForRollback.disableSSHAccess(box.id)
          } catch (disableErr) {
      ...
      try {
        await this.sshAccessRepository.delete({ boxId: box.id, id: Not(sshAccess.id) })
      } catch (deleteErr) {
        ...
        if (legacyDisableAdapter) {
          try {
            await legacyDisableAdapter.disableSSHAccess(box.id)
          } catch (disableErr) {
    ...
    try {
      ...
      if (token) {
        const revokedRow = await this.sshAccessRepository.findOne({
          where: { boxId: box.id, token },
          select: ['id', 'unixUser'],
        })
        revokedIsRealSsh = revokedRow?.unixUser !== null && revokedRow?.unixUser !== undefined
      } else {
        // All-tokens revoke: real-SSH if any token for this box has a unixUser.
        const realSshCount = await this.sshAccessRepository.count({
          where: { boxId: box.id, unixUser: Not(IsNull()) },
        })
      ...
      const remainingRealSsh = token
        ? await this.sshAccessRepository.count({
            where: { boxId: box.id, unixUser: Not(IsNull()), token: Not(token) },
          })
        : 0
      ...
      if (remainingRealSsh === 0 && box.runnerId && (await this.redisLockProvider.isLockOwned(lockKey, lockToken))) {
        const runner = await this.runnerService.findOne(box.runnerId)
        if (runner) {
          const adapter = await this.runnerAdapterFactory.create(runner)
          if (revokedIsRealSsh) {
            ...
            adapter.disableSSHAccess(box.id).catch((e: unknown) => {
              this.logger.warn(`[revokeSshAccess] best-effort disable for box ${box.id} failed: ${e}`)
            })
          }
        } else if (revokedIsRealSsh) {
          // Fail closed: the runner record is missing (e.g. deregistered) but this
          // box has real-SSH state that needs tearing down. Falling through to the
          // DB delete below would produce a false revocation — DB token gone but
          // runner-side sshd/port-forward still live if the runner reappears.
          throw new BadRequestError(
            `Cannot revoke real-SSH access for box ${box.id}: runner ${box.runnerId} not found`,
          )
        }
      }
  ...
  async validateSshAccess(token: string): Promise<SshAccessValidationDto> {
    ...
    if (!sshAccess) {
      return { valid: false, boxId: '', unixUser: null }
    }

    const isExpired = sshAccess.expiresAt < new Date()
    if (!isExpired) {
      if (!sshAccess.box) {
        // Box was deleted concurrently between token issuance and this validation call.
        // The token is technically unexpired but there is nothing to route to — treat as invalid.
        return { valid: false, boxId: '', unixUser: null }
      }
      return { valid: true, boxId: sshAccess.box.id, unixUser: sshAccess.unixUser }
    }

    // Token is expired. Clean up runner SSH under the per-box lock so we don't
    // race a concurrent createSshAccess that is between enableSSHAccess and DB save.
    if (sshAccess.box && sshAccess.box.runnerId) {
      try {
        const lockKey = this.sshLockKey(sshAccess.box.id)
        const lockToken = await this.redisLockProvider.waitForLockOwned(lockKey, 90)
        try {
          ...
          const remainingCount = await this.sshAccessRepository.count({
            where: { boxId: sshAccess.box.id, unixUser: Not(IsNull()) },
          })
          ...
          if (remainingCount === 0 && (await this.redisLockProvider.isLockOwned(lockKey, lockToken))) {
            ...
            if (runner) {
              const adapter = await this.runnerAdapterFactory.create(runner)
              try {
                await adapter.disableSSHAccess(sshAccess.box.id)
              } catch (disableErr) {
    ...
    } else if (sshAccess.box) {
      // Runner-less box (exec-bridge token, unixUser=null): no runner SSH to disable,
      // but the expired row still needs to be removed to prevent unbounded DB growth.
      try {
        await this.sshAccessRepository.delete({ id: sshAccess.id })
      } catch (err) {
        this.logger.error('Failed to delete expired SSH token for runner-less box', err)
      }
    } else {
      // Box relation is null — box was concurrently deleted. No runner SSH to
      // disable, but the orphaned SSH-access row must still be removed to prevent
      // unbounded DB growth.
      try {
        await this.sshAccessRepository.delete({ id: sshAccess.id })
      } catch (err) {
        this.logger.error('Failed to delete expired SSH token for deleted box', err)
      }
    }
